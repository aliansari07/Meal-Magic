...
typedef struct{ 			
	...
	int i,x,again,total,tables,quantity,loginv=0,b=0,l=0,d=0,sumtotal=0;	 
	char choice,id1;	
	int* lptr=&loginv;						   			
...
void registration(customer cust){				
	...
	if(fptr==NULL)
   {
    ...
    printf("Enter ID: ");
	scanf("%s",cust.id);
	while(strlen(cust.id)!=13){
		printf("Incorrect ID length\n");
		goto again;}
	printf("\nFirst name: ");
	fflush(stdin);
	scanf("%[^\n]%*c",&cust.firstname);
	...
	customer login_customer(customer st){ 		
		...
		if (index != -1){
			*lptr=1;
			...
			fclose(fptr);
            printf("\nLogin Successfull\nWelcome Mr/Ms %s to our Restaurant.",(st.firstname));}
		else{
			int attempts;
...
int search(customer cust){
	...
	if(fptr==NULL)
   {
	...
	for(i=1; i<=records ;i++)
	{
		fscanf(fptr,"%s %s %s %s %s %s %s", id,firstname, lastname,number,email,address,gender);
...
void viewall(){
	...
	fptr=fopen("textfile.txt","r");	
  	if(fptr==NULL)												
   	{
      printf("\nNo data stored in the file!\n");
	}
	if (fptr!=NULL){
    printf("\n\t\tDetails of all Customers in the system");
	printf("\n\t\t**************************************\n");
